...
from new_roc_curve_pixel import *
from new_detection_pixel import detection
...
original = 'lena.bmp'
watermarked = 'watermarked.bmp'

mark, wd = embedding(original)
cv2.imwrite("watermarked.bmp",wd)
...
attacked.append(blur(img, 17))
attacked.append(awgn(img, 50, 66))
attacked.append(resizing(img, 0.01))

for i, a in enumerate(attacked):
    aName = 'attacked-%d.bmp' % i
    cv2.imwrite(aName, a)
    tr, w = detection(original, watermarked, aName)
    if tr == 1:
        c += 1
        ws.append(w)
if c > 0:
...
c = 0
for f in files:
    tr, w = detection(original, watermarked, f)
    if tr == 1:
        c += 1
if c > 0:
