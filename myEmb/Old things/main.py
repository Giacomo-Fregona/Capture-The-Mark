...
s = 1
borders = [[0, j*blocksize, s*0.85*2.4,0] for j in range(1,63)] + [[63, j*blocksize, s*0.85*2.4,0] for j in range(1,63)] + [[i*blocksize, 0, s*0.85*2.4,0] for i in range(1,63)] + [[i*blocksize, 63 , s*0.85*2.4,0] for i in range(1,63)]
borders += [[0,0,s*2.4,0],[0,512-blocksize,s*2.4,0],[512-blocksize,0,s*2.4,0],[512-blocksize,512-blocksize,s*2.4,0]]
borders += [[blocksize,blocksize,s*0.81*2.4,0],[blocksize,512-2*blocksize,s*0.81*2.4,0],[512-2*blocksize,0,s*0.81*2.4,0],[512-2*blocksize,512-2*blocksize,s*0.81*2.4,0]]
borders += [[blocksize, j*blocksize, s*0.63*2.4,0] for j in range(2,62)] + [[62, j*blocksize, s*0.63*2.4,0] for j in range(2,62)] + [[i*blocksize, 1 , s*0.63*2.4,0] for i in range(2,62)] + [[i*blocksize, 62 , s*0.63*2.4,0] for i in range(2,62)]
middle = [[i*blocksize,j*blocksize,s*1,0] for i in range(2,62) for j in range(2,62)]



our_ones=[]
our_zeros = []
for i in range(mark_repetitions*mark_size):
    if real_mark[i%mark_size]==1:
        our_ones.append(i)
    else:
        our_zeros.append(i)
for i in range(mark_repetitions*mark_size,n_blocks**2):
    our_zeros.append(i)
random.seed(21)
random.shuffle(our_ones)
random.shuffle(borders)
random.shuffle(middle)
position_list = borders+middle
for i in range(480):
    position_list[i][3] = our_ones[i]
for i in range(480,n_blocks**2-len(our_ones)+480):
    position_list[i][3] = our_zeros[i-480]
for i in range(n_blocks**2-len(our_ones)+480,n_blocks**2):
    position_list[i][3] = our_ones[i-(n_blocks**2-len(our_ones)+480)+480]

def f(i):
    return i[3]

position_list.sort(key = f)
#position_list = [[pos[0],pos[1],1] for pos in position_list]
...
def create_scores_labels(n_samples=50):
    ...
    while sample < n_samples:
        ...
        #print("\nRecovered similarity = {}".format(sim))
        scores.append(sim)
        labels.append(1)
