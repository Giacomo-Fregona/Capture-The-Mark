...
#list of the positions and security values in which to insert (and retrieve) the watermark bits. We should substitute that with our carefully random generated one. Security value = number that we add to each bit in case we are representing a one
...
lena = cv2.imread('lena_pixel.bmp', 0)
lena = np.float32(lena)/255.0
...
def create_scores_labels(n_samples = 50):
    ...
    imageList = []
    for i in range(101):
        #lena = np.float32(cv2.imread('sample-images/{num:04d}.bmp'.format(num = i), 0)) / 255.0
        #np.asarray(Image.open('lena_pixel.bmp'), dtype=np.uint8)
        imageList.append(np.float32(cv2.imread('sample-images/{num:04d}.bmp'.format(num = i), 0)) / 255.0)
